  ...
  (
   ...
   const uint64_t txtPos //!< Txt-position of "ch" (0base).
    ...
    if (pos + 1 != drle_.getWeightFromIdxM(idxM)) {
      ...
      if (idxS != BTreeNode<B>::NOTFOUND) { // Succcessor with "ch" was found.
        ...
        if (reinterpret_cast<uintptr_t>(nextRootS) != BTreeNode<B>::NOTFOUND) {
          idxS = reinterpret_cast<uintptr_t>(nextRootS->getLmBtm()) * B + 1;
          succSamplePos_ = drle_.getAssoc(drle_.idxS2M(idxS)) + 1;
        }
      }
    }
  ...
  (
   ...
   const uint64_t ch
    ...
    uint64_t idxS;
    {
      ...
      l_in_drle += drle_.parentS(btmS)->calcPSum(0, drle_.idxInSiblingS(btmS), true);
    }
